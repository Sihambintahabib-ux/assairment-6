...
const showCategory = (categories) => {
    categories.forEach((cat) => {
        categoryContainer.innerHTML += `
            <li id="${cat.id}" class="hover:border-b-4 hover:border-red-600 border-red-600 cursor-pointer">${cat.category_name}</li>
        `;
    });
...
const showNewsByCategory = (plants) => {
    ...
    plants.forEach((plants) => {
        ...
        <div class="border  border-gray-300 rounded-lg">
            <div>
             <img src="${plants.image.srcset[5].url}"/>
            </div>
            <div id="${plants.id}" class="p-2">
                <h1 class="font-extrabold">${plants.name}</h1>
